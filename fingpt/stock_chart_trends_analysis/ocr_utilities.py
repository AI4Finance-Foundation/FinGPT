...
def convert_compact_number(val):
    ...
    try:
        if val.endswith('k'): 
            print(f"Original: {val} | Normalized: {val} → {float(val[:-1]) * 1e3}")
...
def extract_volume(texts):
    ...
    for t in texts:
        # Catch confident patterns first
        ...
        # Fallback: collect all compact numbers
        bars = re.findall(r'\b\d{1,3}(?:,\d{3})*(?:\.\d+)?[kKmMbB]\b', t)
        fallback.update(bars)

    # Use confident matches if available
    preferred_vals = [convert_compact_number(v) for v in preferred if convert_compact_number(v)]
    if preferred_vals:
        return max(preferred_vals)

    # Only now filter fallback against time intervals
    fallback_vals = [
        convert_compact_number(v)
        for v in fallback
        if v.lower() not in interval_keywords and convert_compact_number(v)
    ]
